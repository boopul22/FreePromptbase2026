...
import { isTagIndexable } from '../data/tag-seo';
...
export const GET: APIRoute = async () => {
	const tags = getAllTags().filter((t) => isTagIndexable(t.slug));
	...
	const staticLastmod: Record<string, string | undefined> = {
		...
		'/news': latestNewsDate,
		'/skills': latestPromptDate,
	};

	// Prompts win the shared /<slug> namespace — skip any tag whose slug is also a prompt.
	const promptSlugs = new Set(prompts.map((p) => p.slug));

	const entries: Entry[] = [
		...STATIC_PATHS.map((p) => ({ loc: p, lastmod: staticLastmod[p] })),
		...categories.map((c) => ({ loc: `/category/${c.slug}`, lastmod: catLastmod.get(c.slug) })),
		...tags
			.filter((t) => !promptSlugs.has(t.slug))
			.map((t) => ({ loc: `/${t.slug}`, lastmod: latestPromptDate })),
		...prompts.map((p) => {
